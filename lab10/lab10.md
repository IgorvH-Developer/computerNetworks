...
адрес компьютера - 192.168.0.115
...
- В IP заголовки: 20 байт
- Полезная нагрузка: 8 байт udp + 28 байт Data
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Увеличивается на 1 с каждым новым пакетом
...
TTL остаётся неизменным.

Identification изменяется на один
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Была фрагментация. Пакеты делились по 1500 байт
      - Первый фрагмент: 1480 байт данных + 20 байт заголовка
      - Второй фрагмент: 1480 байт + 20 байт заголовка
      - Третий фрагмент: 540 байт = 20 байт заголовка + 8 байт UDP заголовка + 512 бат данных

![images/img_7.png](images/img_7.png)

   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Flags (во фрагментах More Fragments)
      - Fragment Offset
      - Total Length
      - Checksum
